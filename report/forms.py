...
class IncidentReportForm(forms.ModelForm):
    ...
    incident_type = forms.CharField(
        widget=forms.Select(choices=IncidentTypeChoice.choices)
    )
    ...
    root_cause = forms.CharField(
        widget=forms.Textarea(attrs={"rows": "6", "cols": "50"}), required=False
    )
...
IncidentReportFileForm = forms.modelform_factory(
    IncidentReportFiles,
    fields=["file", "filename", "incident_report"],
    widgets={
        "filename": forms.HiddenInput(),
        "incident_report": forms.HiddenInput(),
    },
)
